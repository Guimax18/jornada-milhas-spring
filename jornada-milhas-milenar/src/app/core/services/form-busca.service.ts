...
import { DadosBusca, Estado, MinMaxPrice } from '../types/type';
...
export class FormBuscaService {
  ...
  obterDadosBusca(): DadosBusca {
    ...
    const dadosBusca: DadosBusca = {
      ...
      origemId: this.obterControle<Estado>('origem').value.id,
      destinoId: this.obterControle<Estado>('destino').value.id,
      tipo: this.obterControle<string>('tipo').value,
      minMaxPrice: {
        min: this.obterControle<number>('precoMin').value,
        max: this.obterControle<number>('precoMax').value
      }
    }
    ...
    if(precoMinControl.value){
      dadosBusca.minMaxPrice.min = precoMinControl.value
    }
    const precoMaxControl = this.obterControle<number>('precoMin')
    if(precoMaxControl.value){
      dadosBusca.minMaxPrice.max = precoMaxControl.value
    }
    return dadosBusca
  }
